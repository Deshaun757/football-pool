...
try {
  ...
  await call(member.cookie,'/api/admin/weeks','GET',undefined,403);
  await call(member.cookie,`/api/admin/weeks/${weekId}/games`,'GET',undefined,403);
  assert.ok((await call(admin.cookie,'/api/admin/weeks')).data.some(w=>w.id===weekId));
  ...
  const matchups=(await call(admin.cookie,`/api/admin/weeks/${weekId}/games`)).data;
  assert.equal(matchups.length,1);
  assert.equal(matchups[0].id,game.insertId);
  assert.ok(matchups[0].awayName && matchups[0].homeName);
  assert.equal(matchups[0].isTiebreaker,1);
  assert.equal(matchups[0].homeScore,null);
  ...
  assert.deepEqual(boardA.winners,[],'No winners before finalization');
  ...
  const winnersA=(await call(member.cookie,`/api/groups/${a}/picks-board`)).data.winners;
  const winnersB=(await call(member.cookie,`/api/groups/${b}/picks-board`)).data.winners;
  assert.deepEqual(winnersA.map(w=>w.entryId),[entryA.id]);
  assert.deepEqual(winnersB.map(w=>w.entryId),[entryB.id]);
  assert.equal(winnersA[0].correctPicks,1);
  assert.equal(winnersA[0].tiebreakerDifference,0);
